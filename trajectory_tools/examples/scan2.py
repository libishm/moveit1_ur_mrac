...
start_srv_req.rgbd_params.depth_trunc = 0.35
...
stop_srv_req.mesh_filepath = "/home/libish/libish14.ply"
...
def robot_program():
    ...
    start = (0.0, -pi / 2.0, pi / 2.0, -pi, -pi / 2.0, pi)
    pose1 = Pose(
        position=Point(0.9, -0.5, 0.3), orientation=Quaternion(0.0, 1.5, 0.8, 0.0)
    )
    pose2 = Pose(
        position=Point(0.9, 0.5, 0.3), orientation=Quaternion(0.0, 1.5, -0.8, 0.0)
    )
    ...
    th.sequencer.plan(Ptp(goal=start, vel_scale=0.3, acc_scale=0.5))
    th.sequencer.execute()
    th.sequencer.plan(Ptp(goal=pose1, vel_scale=0.3, acc_scale=0.5))
    th.sequencer.execute()
    ...
    th.sequencer.plan(Lin(goal=pose2, vel_scale=0.1, acc_scale=0.5))
    th.sequencer.execute()
